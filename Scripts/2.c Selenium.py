...
soup.find_all("div", class_="checkbox")  # We look for the checkboxs we are interested in, but there are too many
...
att = "sources.selectSource('AW_DCE')"  # Because of the way that quotes and double-quotes interact, we need to put the value of that attribute in a variable
el = driver.find_element(By.XPATH, r'.//*[@ng-change="' + att + '"]')  # And then look at that attribute like this - note that before closing the first string, we have the double-quotes, which are closed at the beginning of the second string
el = driver.find_element(By.XPATH, r'.//*[contains(text(), "Décisions du Conseil")]')  # An alternative is to realise that the relevant checkbox has this unique text, so we use xPath to get button
...
soup = BeautifulSoup(driver.page_source)  # We recreate a soup, now that the page source has changed, since new content has been dynamically added
table = soup.find_all("table")[-1]  # Collect tables from the page; there are two of them in the page source (and find_all returns a list), and we are interested in the last one, with results. -1  indexing is not ideal, though, we should sort by length maybe

df = pd.read_html(str(table))[-1]  # To make things easier, we convert the table in a panda dataframe, with each row storing data about one decision. Note the 'str' command: it's because the original table is a BeautifulSoup object, and not a string. This method returns a list of dataframes, so make sure to select the last one
...
for x in range(2500):
    ...
    LL.append(pd.read_html(str(table))[-1]  )
