...
df.head(5)
df.columns
df["Lait"]  # You access a particular column by indexing it this way (we'll see further indexing in a few weeks)
df["Lait"].unique()
...
dict_dep = dep_reg.set_index("dep_name").to_dict()["region_name"]   # We create a dictionary from the second dataframe; this requires us to set the index with the keys of the dict: the departments; regions will be the values, as put in the column
...
fr_chart = maps.Departments(human_readable=True)  # Taken straight from documentation online
