...
tableau = []
for el in l[:10] :  # LOOP 2: creating a frame with different value columns
    ...
    if art700:
        ...
        if sum_700:
            ...
            sum_700 = sum_700 = re.sub("(,|\.)\d\d(?=\D)", "", sum_700) #Converting the regex into harmonious format (nouvelle ligne)
            sum_700 = re.sub("[^0-9]", "", sum_700) #Replacing the sum that can be in multiple format by a comparable sum
            sub.append(sum_700)
        else:
...
df.to_csv("/Users/laurinele/PycharmProjects/Legal-Data-Analysis/Final prez 2023/essai"
          ".csv")  # Creating the CSV file
