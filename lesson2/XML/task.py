...
os.chdir("./lesson2/XML/Not so gibberish")  # We go to the main folder that stores all files
...
for file in files:
    ...
    full_date = parsed_date.strftime("%A %d %B %Y")  # Your datateime object can then be transforme (strftime) into a more pleasant date format, again using a pattern
...
df = pd.DataFrame(lists_details)  # Out of the loop, we create a dataframe based on that list of lists
